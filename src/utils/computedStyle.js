...
const computedStyle = (cell, date) => {
    ...
    if (Array.isArray(timeRange)) {
        ...
        if (endDayjs.isAfter(thisWeek)) {
             ...
             * 最后加5为模拟折角效果需要延伸的长度。
             */
            let widthStr = `calc(${throughNum + 1}00% + ${CELL_PADDING * (2 * throughNum + 1) + throughNum + 5}px)`;
            ...
            if (startDayjs.isBefore(date)) {
                ...
                widthStr = `calc(${throughNum + 1}00% + ${CELL_PADDING * (2 * (throughNum + 1)) + throughNum + 10}px)`;
            ...
            let widthStr = `calc(${throughNum + 1}00% + ${CELL_PADDING * (2 * throughNum) + throughNum}px)`;
            const styleObj = {
                style: {
                    width: widthStr,
                    backgroundColor: cell.color.backgroundColor,
            ...
            if (startDayjs.isBefore(date)) {
                widthStr = `calc(${throughNum + 1}00% + ${CELL_PADDING * (2 * throughNum) + throughNum + offset}px)`;
                styleObj.style.width = widthStr;
